...
export async function getAllEtablissement() {
    let requests = [];
    requests[0] = {};
    requests[0].Fields = {};
    requests[0].Fields.Nom_Etablissement_si_Centre_Hosp = '*';
    requests[0].Type = 'find';
    let sortFields = [];
    sortFields[0] = {};
    sortFields[0].FieldName = 'Nom_Etablissement_si_Centre_Hosp';
    sortFields[0].SortOrder = 'Descend';
    let dataEtablissement = [];
    dataEtablissement = await get(
        global.layoutEtablissement,
        'FindRecords',
        requests,
        sortFields
    );
    ...
    for (let i = 0; i < dataEtablissement.length; i++) {
        etablissement[i] = {};
        etablissement[i].No_Etablissement_si_Centre_Hosp =
            dataEtablissement[i].No_Etablissement_si_Centre_Hosp;
        etablissement[i].Nom_Etablissement_si_Centre_Hosp =
            dataEtablissement[i].Nom_Etablissement_si_Centre_Hosp;
    }
...
export function getErrorMessageFM(code) {
    if (Array.isArray(code)) {
        return "Vous n'avez pas les droits pour pouvoir éditer.";
    }
...
export async function getMaladiesData() {
    return await get('Maladie', 'FindAllRecords', false, false, {
        database: 'MAG__Repertoire',
    });
}

export async function getListeMaladieStructurer(dataMaladies) {
    let maladiesTemp = [];
    for (let i = 0; i < dataMaladies.length; i++) {
        maladiesTemp[i] = {};
        maladiesTemp[i].Id = dataMaladies[i].Id;
        maladiesTemp[i].nom = '';
        for (let j = 0; j < dataMaladies[i].nom_t.length; j++) {
            if (dataMaladies[i].nom_t[j].data) {
                if (maladiesTemp[i].nom.length == 0) {
                    maladiesTemp[i].nom = dataMaladies[i].nom_t[j].data;
                } else {
                    maladiesTemp[i].nom += '###' + dataMaladies[i].nom_t[j].data;
                }
            }
...
export async function getAllNamesMaladies(dataMaladies) {
    // let dataMaladies = [];
    let cpt = 0;
    let allNamesMaladies = [];
    let indexAllNamesMaladies = 0;
    for (let i = 0; i < dataMaladies.length; i++) {
        for (let j = 0; j < dataMaladies[i].nom_t.length; j++) {
            if (dataMaladies[i].nom_t[j].data) {
                allNamesMaladies[indexAllNamesMaladies] = dataMaladies[i].nom_t[j].data;
                indexAllNamesMaladies++;
            }
        }
    }

    return allNamesMaladies;
}

export async function getSuccursale() {
    let theReturn = {};
    let requests = [];
    requests[0] = {};
    requests[0].Fields = {};
    requests[0].Fields.Nom_Etablissement_si_Succ = '*';
    requests[0].Fields.No_Etablissement_si_Succ = '*';
    requests[0].Type = 'find';
    let sortFields = [];
    sortFields[0] = {};
    sortFields[0].FieldName = 'Nom_Etablissement_si_Succ';
    sortFields[0].SortOrder = 'Descend';

    const dataEtablissement = await get(
        global.layoutEtablissement,
        'FindRecords',
        requests,
        sortFields
    ...
    let dataSuccursale = [];
    let dataSuccursaleTemp = [];
    for (let i = 0; i < dataEtablissement.length; i++) {
        dataSuccursale[i] = dataEtablissement[i].Nom_Etablissement_si_Succ;
        dataSuccursaleTemp[i] = {};
        dataSuccursaleTemp[i].label =
            dataEtablissement[i].Nom_Etablissement_si_Succ;
        dataSuccursaleTemp[i].id = dataEtablissement[i].No_Etablissement_si_Succ;
    }
    theReturn.listeInfo = dataSuccursaleTemp;
    theReturn.forPicker = dataSuccursale;
    theReturn.succursaleList = dataEtablissement;
    return theReturn;
}

export function getConseillers() {
    return [
        {
            Courriel: 'alain.simoneau@icloud.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Simoneau',
            Prenom: 'Alain',
            fk_Entreprise: '1',
            pk_id: '4',
        ...
        {
            Courriel: 'arodrigues@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Rodrigues',
            Prenom: 'Antonio',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'clafon@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Lafon',
            Prenom: 'Chantal',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'cperron@v-hiculemedia.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Perron',
            Prenom: 'Clément',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'efrancis@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Francis',
            Prenom: 'Élie',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'cercoli@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Ercoli',
            Prenom: 'Cono',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'dmiron@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Miron',
            Prenom: 'Dominique',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'erenaud@v-hiculemedia.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Renaud',
            Prenom: 'Edward',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'erics@creationlogicom.ca',
            Entreprise: 'Magnus Poirier',
            Nom: 'Savard',
            Prenom: 'Eric',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'fvezina@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Vézina',
            Prenom: 'François',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'jlaurent@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Laurent',
            Prenom: 'Jacques',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'jpoirier@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Poirier',
            Prenom: 'Jacques',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'jpalazzo@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Palazzo',
            Prenom: 'John',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'meriodin@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Riodin',
            Prenom: 'Marc Eddy',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'mdiliello@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Di Liello',
            Prenom: 'Mario',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'poirierm@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Poirier',
            Prenom: 'Mélanie',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'mouezzani@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Ouezzani',
            Prenom: 'Mohamed',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'rbelkahla@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Belkahla',
            Prenom: 'Rabah',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'sdesgagnes@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Desgagnés',
            Prenom: 'Sophie',
            fk_Entreprise: '1',
        ...
        {
            Courriel: 'tlumia@magnuspoirier.com',
            Entreprise: 'Magnus Poirier',
            Nom: 'Lumia',
            Prenom: 'Tommy',
            fk_Entreprise: '1',
...
export async function getTypeDeCas() {
    let dataFind = [];
    let requests = [];
    ...
    for (let i = 0; i < dataFind.length; i++) {
        typeCas[i] = dataFind[i].No_Seq_TypeCas + '-' + dataFind[i].Nom_Type_Cas;
    }

    return typeCas;
}

export async function getListeConseillers() {
    let dataFind = [];
    dataFind = await get('VHM_Util', 'FindAllRecords', false, false, {
        database: 'MAG__Systeme',
    });
...
export function chercherPkIdDeConseiller(fullName, listConseiller) {
    let dataConseiller = getConseillers();

    let nomObject = fullName.split(' ');
    let prenom = nomObject[0];
    let nom = nomObject[1];
    let pkId = '';

    for (let i = 0; i < listConseiller.length; i++) {
        if (
            listConseiller[i].UTIL_Prenom == prenom &&
            listConseiller[i].UTIL_Nom == nom
        ) {
            pkId = listConseiller[i].No_Seq_Util;
        }
    }
...
export function chercherNomConseiller(pk_id) {
    let dataConseiller = getConseillers();
    let fullNameReturn = 'Remplir';

    for (let i = 0; i < dataConseiller.length; i++) {
        if (dataConseiller[i].pk_id == pk_id) {
            fullNameReturn = dataConseiller[i].Prenom + ' ' + dataConseiller[i].Nom;
        }
    }
